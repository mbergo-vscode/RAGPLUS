...
import { Play, Database, RefreshCw, Terminal, ChevronRight, RotateCcw, Users, Image as ImageIcon, Video, Upload, Loader2, Zap, RefreshCcw, Sparkles } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className="h-screen flex flex-col font-sans text-slate-200">
      {/* Header with Gradient */}
      <header className="border-b border-white/10 bg-gradient-to-r from-navy-900 to-slate-900 px-6 py-4 flex items-center justify-between shrink-0 shadow-lg relative z-20">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">Multi-tenant Architecture Explorer</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-lg border border-white/10 hover:border-white/20 transition-colors">
                <Users className="w-4 h-4 text-indigo-400" />
                <span className="text-xs text-slate-400 font-semibold uppercase">Tenant:</span>
                <select 
            ...
            <a href="https://github.com/google-gemini" target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:text-blue-300 transition font-medium">
      ...
      {/* Main Content Grid */}
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden bg-navy-900/50">
        
        {/* Left: Visualization & Controls */}
        <div className="lg:col-span-8 p-6 flex flex-col gap-6 overflow-y-auto">
          
          {/* Controls Bar - Lighter background */}
          <div className="bg-slate-800/40 backdrop-blur-md border border-white/10 rounded-2xl p-5 flex flex-col gap-4 shadow-xl">
            
            {/* Top Row: Query Input */}
            <div className="w-full">
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-wide mb-2">User Query Simulation</label>
              <input 
                type="text" 
                value={userQuery}
                onChange={(e) => setUserQuery(e.target.value)}
                disabled={simulationMode !== null}
                className="bg-navy-900/80 border border-white/10 text-sm rounded-lg px-4 py-3 w-full focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition disabled:opacity-50 text-white shadow-inner"
                placeholder="Ask a question..."
              />
            </div>

            {/* Bottom Row: Toggles & Actions */}
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between border-t border-white/10 pt-4">
                 {/* Feature Toggles */}
                 <div className="flex items-center gap-3">
                    <button 
                        onClick={() => !simulationMode && setSimulateRedisHit(!simulateRedisHit)}
                        disabled={simulationMode !== null}
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border transition shadow-sm ${
                            simulateRedisHit 
                 ...
                 <div className="flex gap-2">
                    {!simulationMode ? (
                        ...
                        <button 
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-emerald-600/90 hover:bg-emerald-500 text-white text-sm font-semibold rounded-lg transition shadow-lg shadow-emerald-900/20 border border-emerald-500/50"
                        >
                        ...
                        <button 
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-indigo-600/90 hover:bg-indigo-500 text-white text-sm font-semibold rounded-lg transition shadow-lg shadow-indigo-900/20 border border-indigo-500/50"
                        >
                        ...
                        <button 
                            ...
                            className="flex items-center gap-2 px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold rounded-lg transition shadow-lg shadow-blue-900/20 animate-pulse-fast border border-blue-400/50"
                        >
          ...
          <div className="flex-1 min-h-[400px] flex flex-col">
             <div className="flex items-center justify-between mb-2">
               <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wide">Architecture Diagram</h2>
               {simulationMode && (
                 <span className="text-xs bg-emerald-500/10 text-emerald-400 px-3 py-1 rounded-full border border-emerald-500/20 font-mono font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-64">
             ...
             {/* Console Log */}
             <div className="bg-navy-900 border border-white/10 rounded-xl p-0 font-mono text-xs overflow-hidden flex flex-col shadow-xl">
              <div className="bg-navy-800 px-4 py-2 border-b border-white/5 flex items-center gap-2">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-1.5 scrollbar-thin scrollbar-thumb-slate-700">
                {consoleLogs.map((msg, i) => (
                  <div key={i} className={`truncate font-medium ${i === 0 ? 'text-emerald-400' : 'text-slate-400'}`}>
                    <span className="opacity-50 mr-2">$</span>{msg}
                  </div>
                ))}
                {consoleLogs.length === 0 && <span className="text-slate-600 italic opacity-50">System ready. Waiting for pipeline initiation...</span>}
              </div>
            </div>
          </div>
        </div>

        {/* Right: Info Panel - Lighter, blurred background */}
        <div className="lg:col-span-4 h-full border-l border-white/5 bg-slate-900/80 backdrop-blur-sm relative z-10">
